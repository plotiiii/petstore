...
from com.jcl.petstore.dao.product_dao import ProductDao
from com.jcl.petstore.ui.my_frame import MyFrame
from com.jcl.petstore.ui.cart_frame import CartFrame
from com.jcl.petstore.ui.product_list_gridtable import ProductListGridTable
...
class ProductListFrame(MyFrame):
    ...
    def createtopbox(self):
        ...
        #创建静态文本
        pc_st = wx.StaticText(parent=self.contentpanel, label='选择商品类别：',
                              style=wx.ALIGN_RIGHT)
        #创建按钮对象
        search_btn = wx.Button(parent=self.contentpanel, label='查询')
        reset_btn = wx.Button(parent=self.contentpanel, labell='重置')
        ...
        self.Bind(wx.EVT_BUTTON, self.resert_btn_onclick, reset_btn)
    ...
    def addcart_btn_onclick(self, event):
        ...
        if productid in self.cart.keys():  #判断购物车已经有该商品
            #获得商品数量
    ...
    def selectrow_handler(self, event):
        ...
        if srowidx >= 0:
            ...
            #商品市场价格
            slistprice = '商品单价：￥{0:.2f}'.format(self.selecteddata['listprice'])
            listprice_st = self.FindWindowByName('listprice')
            listprice_st.SetLabelText(slistprice)

            # 市场价格
            sunitcost = '商品市场价：￥{0:.2f}'.format(self.selecteddata['unitcost'])
